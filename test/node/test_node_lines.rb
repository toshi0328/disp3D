...
class NodeLinesTestCase < MiniTest::Unit::TestCase
  def setup
  ...
  def test_initalize
    ...
    node_with_name = Disp3D::NodeLines.new(geom, :test_array)

    node_without_name = Disp3D::NodeLines.new(geom_ary)
    ...
    assert_raises ArgumentError do
      dmy = Disp3D::NodeLines.new(Vector3.new())
    end
    assert_raises ArgumentError do
      dmy = Disp3D::NodeLines.new([Vector3.new(), Vector3.new(1,0,0)])
    end
    assert_raises ArgumentError do
      dmy = Disp3D::NodeLines.new(geom_ary, Vector3.new())
    end
  end
