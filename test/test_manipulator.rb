...
class ManipulatorTestCase < MiniTest::Unit::TestCase
  ...
  def test_center_pos
    center_pos = @gl_view_has_line.manipulator.center_pos(@gl_view_has_line.world_scene_graph)
    assert_equal(Vector3.new(0.5, 0.5, 0.5), center_pos)

    center_pos = @gl_view_has_point.manipulator.center_pos(@gl_view_has_point.world_scene_graph)
    assert_equal(Vector3.new(3,2,1), center_pos)
  end

  def test_bb_radius
    radius = @gl_view_has_line.manipulator.bb_radius(@gl_view_has_line.world_scene_graph)
    assert_in_delta(0.5*Math.sqrt(3.0), radius, 1.0e-8)

    radius = @gl_view_has_point.manipulator.bb_radius(@gl_view_has_point.world_scene_graph)
